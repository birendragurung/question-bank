...
class Database{
	...
	private $db_name = "question_bank";	// Change as required
	...
	private $myconn;   //mysqli object for interacting with the database
	// Function to make connection to database
	public function connect(){
		if(!$this->con){
			$this->myconn = new mysqli($this->db_host,$this->db_user,$this->db_pass);  // new mysqli object with variables defined at the start of Database class
            if($this->myconn){
            	$seldb = $this->myconn->select_db($this->db_name);
                if($seldb){
                	$this->con = true;
	...
	public function sql($sql){//returns the result in the form of array[][]
		...
		if($query){
			...
			for($i = 0; $i < $this->numResults; $i++){
				$r      = $query->fetch_array();//returns the numeric indexed key array to variable r
               	$key    = array_keys($r);
    ...
    public function query($sql){//returns the result in the form of object
        $this->result   = $this->myconn->query($sql);
        $this->myQuery  = $sql;// pass back the SQL
        if ($this->result) {
            $this->numResults = $this->result->num_rows;
            return $this->result;   
        }
        else return false;//query unsuccessful
    }
